...
#pragma warning disable CS8605 // Unboxing a possibly null value.
        ...
        public void Fill32(UInt32 InclusiveStartAddress, UInt32 ExclusiveEndAddress, UInt32 value)
        {
            for (UInt32 address = InclusiveStartAddress; address < ExclusiveEndAddress; address += 4)
            {
                if (!Memory.ContainsKey(address))
                {
                    Memory[(UInt32)(address)] = (byte)(value & 0xFF);
                 }
                if (!Memory.ContainsKey((UInt32)(address + 1)))
                    {
                    Memory[(UInt32)(address + 1)] = (byte)(value >> 8);
                }
                if (!Memory.ContainsKey((UInt32)(address + 2)))
                {
                    Memory[(UInt32)(address + 2)] = (byte)(value >> 16);
                }
                if (!Memory.ContainsKey((UInt32)(address + 3)))
                {

                    Memory[(UInt32)(address + 3)] = (byte)(value >> 24);
                }
        ...
        /// <param name="ExclusiveEndAddress"></param>
        /// <returns></returns>
        public string to16BitArray(UInt32 InclusiveStartAddress, UInt32 ExclusiveEndAddress, bool CommentAddresses = false)
        {
            if ((InclusiveStartAddress & 0x03) != 0)
            {
                return ($"Inclusive Start address 0x{InclusiveStartAddress:X8} is not on word boundary.");
            ...
            for (UInt32 i = InclusiveStartAddress; i < ExclusiveEndAddress; i +=2)
            {
                UInt16 newData = (byte)Memory[(UInt32)i];
                newData += (UInt16)(((byte)Memory[(UInt32)(i + 1)]) << 8);
